...
function mount() {
  ...
  if (typeof FontFace !== 'undefined' && document.fonts && ![...document.fonts].some(face => face.family === 'NightmareHero')) {
    const face = new FontFace('NightmareHero', `url("${chrome.runtime.getURL('fonts/Nightmare_Hero_Normal.ttf')}")`);
  ...
  function pause() { video?.pause(); held.clear(); crowd.stop(); }
  function toggle() {
  ...
  async function start() {
    ...
    try {
      ...
      game = new Game(chart ? validateChart(chart, video.duration) : practiceChart(video.duration, settings.bpm, settings.firstBeat, settings.difficulty), settings.chords);
  ...
  function tick(now) {
    ...
    if (ad || listening || minimized || !started || !video || video.paused || video.seeking || video.readyState < 3) crowd.stop();
    ...
    if (ad) { $('.resume').textContent = 'Waiting for YouTube’s ad to finish…'; lastAd = true; }
      ...
      if (game && video && !minimized) {
        const time = video.currentTime - settings.offset / 1000;
        if (started && !video.paused && !video.seeking && video.readyState >= 3) { game.update(time); game.updateHolds(time, KEYS.map(key => held.has(key))); crowd.update(game, time); }
